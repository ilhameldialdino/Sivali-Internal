...
import listEmployees from "../../data/listEmployee";
import { Link } from 'react-router-dom';
import { Routes } from "../../routes";

const TableRow = ({ employeeId, date, name, amount, status}) => {
...
const ListEmployee = () => {
  const totalListEmployees = listEmployees.length;

  return (
    <Container className="px-0">
      <Row className="d-flex flex-wrap flex-md-nowrap align-items-center py-4">
        <Col className="d-block mb-4 mb-md-0">
          <h1 className="h2">List Employee</h1>
          <p className="mb-0">
        ...
        <Col className="d-flex justify-content-end">
          <Button variant="primary" className="m-1" as={Link} to={`/dashboard/create-transaction`}>
            <FontAwesomeIcon icon={faPlus} className="me-2" /> New Employee
          </Button>
        </Col>
      </Row>
      <Card border="light" className="table-wrapper table-responsive shadow-sm">
        <Card.Body className="pt-0">
          <Table hover className="user-table align-items-center">
            ...
            <tbody>
              {listEmployees.map(t => <TableRow key={`transaction-${t.invoiceNumber}`} {...t} />)}
            </tbody>
          </Table>
          <Card.Footer className="px-3 border-0 d-lg-flex align-items-center justify-content-between">
            ...
            <small className="fw-bold">
              Showing <b>{totalListEmployees}</b> out of <b>25</b> entries
            </small>
          </Card.Footer>
        </Card.Body>
      </Card>
